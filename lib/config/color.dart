import 'package:flutter/material.dart';

class CoodigColors {
  static const Color primary = Color(0xFF4DC0B2);
  static const Color secondary = Color(0xFF202F55);
  static const Color buttonPrimary = Color(0xFF4DC0B2);
  static const Color buttonSecondary = Color(0xFF448AFF);
  static const Color buttonGrey = Color(0xFFA6A6A6);
  static const Color grey = Color(0xFFA6A6A6);
  static const Color background = Color(0xFFF1F1F1);
  static const Color iosLoading = Color(0xFF2B64F6);
  static const Color androidLoading = Color(0xFF448AFF);
  static const Color correctCard = Color(0xFFE6FFEC);
  static const Color incorrectCard = Color(0xFFFFE6E9);
  static const Color questionCard = Color(0xFF202F55);
  static const Color success = Color(0xFF65B776);
  static const Color error = Color(0xFFF87B72);
}
